-- ============================================================================
-- global_events Impact Analysis: Sentiment, Volatility, Recovery by Sector
-- ============================================================================
-- Business Context:
--   Assess how major global_events (financial crises, elections, wars, etc.) impact
...
-- --------------------------------------------------------------------------
DESCRIBE dim_global_events;

UPDATE dim_global_events SET start_date = STR_TO_DATE(start_date, '%d-%m-%Y');
UPDATE dim_global_events SET end_date   = STR_TO_DATE(end_date, '%d-%m-%Y');

ALTER TABLE dim_global_events
...
-- Drop existing views
DROP VIEW IF EXISTS global_events_impact_on_markets;
...
WITH event_returns AS (
    ...
    GROUP BY event.global_event_id, event.sentiment
)
SELECT
    sentiment,
...
-- Insight: Market reacts more to positive global_events than negative global_events.

-- --------------------------------------------------------------------------
-- 2. Market Volatility Around global_events (20-day rolling)
-- --------------------------------------------------------------------------
...
CREATE VIEW global_events_impact_on_markets AS
...
FROM global_events_impact_on_markets
GROUP BY sentiment;

/*
Observations:
  1. Sentiment drives returns.
  2. Negative global_events impact markets for >3 months on average.
